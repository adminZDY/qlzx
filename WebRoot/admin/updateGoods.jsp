...
<script type="text/javascript">
  ...
  <div style="background:url(../images/titlebar_div_bg.jpg) repeat-x bottom; margin-top:5px; height: 25;">
   	<img src="../images/doc_red.gif">查看/修改商品信息
  </div>
  <form name="upform" action="GoodsManage?op=update&goodsid=<%=goodsinfo.getGoodsId() %>" method="POST">
    <table id="table_1" width="100%" height= "400">
    	<tr>
    		<td >商品名称:</td>
    		<td><input type="text" name="goodsname" value="<%=goodsinfo.getGoodsName()%>" placeholder = "请输入商品名称" 
    		...
    		<select name ="goodstype">
    			<%
    				for(GoodsType type:goodstype)
    				{
    					...
    					if(type.getTypeName().equals(goodsinfo.getGoodstype().getTypeName()))
    					{%>
    	...
    	<tr>
    		<td>商品价格/折扣:</td>
    		<td>价格：<input type="text" name="goodsprice" value="<%=goodsinfo.getPrice()%>" size="5" onkeydown="clear_span('span_price')"
    		 onblur="checkPrice('span_price')" placeholder = "价格"/>
    		折扣：<input type="text" name="discount" value="<%=goodsinfo.getDiscount()%>" size="5" onkeydown="clear_span('span_price')" 
    	...
    	<tr>
    		...
    		if(goodsinfo.getStatus() == 0){ 
    			out.println("<input type='checkbox' name='stastus' value='0' checked='checked'>");
    			}else{
    			out.println("<input type='checkbox' name='stastus' value='0'>");
    			}
    		%>上架
    		<% if(goodsinfo.getIsRecommend() == 0){ 
    			out.println("<input type='checkbox' name='isrecommend' value='0' checked='checked'>");
    			}else{
    			out.println("<input type='checkbox' name='isrecommend' value='0'>");
    			}
    		%>推荐
    			<% if(goodsinfo.getIsNew() == 0){ 
    			out.println("<input type='checkbox' name='isnew' value='0' checked='checked'>");
    			}else{
    			out.println("<input type='checkbox' name='isnew' value='0'>");
    			}
    		%>新品
    		</td>
    	</tr>
    	<tr>
    		<td>商品图片:</td>
    		<td>
    			<input type="text" style="float: left;" size="40" id="txt" name="photo" value="<%=goodsinfo.getPhoto()%>"/>
    			<input type="file" style="width:70px; float: left; margin-left:10px;" onchange="fun(this)"/>
    		</td>
    	</tr>
    	<tr >
    		<td >商品描述:</td>
    		<td >
    			<textarea cols="55" rows="6" name="remark"><%=goodsinfo.getRemark() %></textarea>
    		</td>
    	</tr>
    	<tr>
    		<td>商品详细</td>
    		<td>
    			<textarea cols="60" rows="8" name="content" id="scpg"><%=goodsinfo.getDetailed()%></textarea>
    		</td>
    	</tr>
    	<tr>
    		<td></td>
